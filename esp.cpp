...
namespace offsets
{
    ptrdiff_t p_entity_list = 0x178DC58; 
...
{
    SetConsoleTitleA("CS2-Cheat Made by: github/hidenway");
    ...
    fetchOffsets();
    printf("%sCS2-Cheat Made by: github/hidenway - Press F1 to enable/disable%s\n", COLOR_BLUE, COLOR_RESET);
    while (true)
    {
        ...
        for (int i = 1; i < 64; i++)
        {
            ...
            if (p_cs_player_pawn == 0)
                continue;
            if (!glow_enabled)
                memory_write<float>(cs2_process_handle, p_cs_player_pawn + offsets::m_fl_detected_by_enemy_sensor_time, 0.f); // выкл
            else
                memory_write<float>(cs2_process_handle, p_cs_player_pawn + offsets::m_fl_detected_by_enemy_sensor_time, 100000.f); // вкл
        }
